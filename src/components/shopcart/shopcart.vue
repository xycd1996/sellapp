<template>
  <div class="shopcart">
    ...
    <transition name="shopcart-list-action">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      this.$bus.on('add', (el) => {
        // 监听从cartcontrol传递过来的add事件，触发后执行括号内方法，el为传递过来的参数event.target
        this.$nextTick(() => {
          this.drop(el)
        })
      })
    ...
    methods: {
      pay() {
        ...
        window.alert(`支付${this.totalPrice}元另需配送费¥${this.deliveryPrice}元`)
      },
      clear() {
        this.selectFoods.forEach((food) => {
          food.count = 0
...
<style lang="stylus" rel="stylusheet/stylus" scoped>
...
.shopcart
  ...
  .shopcart-list
    ...
    &.shopcart-list-action-enter-active, &.shopcart-list-action-leave-active
      transition all 0.5s
      transform translate3d(0, -100%, 0)
    &.shopcart-list-action-enter, &.shopcart-list-action-leave-to
  ...
  .list-mask
    ...
    &.mask-action-enter-active, &.mask-action-leave-active
      transition all 0.5s
      opacity 1
